...
mlir::Operation *OPSBuilder::buildParLoopOp(const LoopDesc &loop) {
  ...
  auto kernelPtrAttr = builder.getI64IntegerAttr(
      static_cast<int64_t>(loop.kernel_token));
  ...
  auto opState = mlir::OperationState(loc, ops_mlir::ops::ParLoopOp::getOperationName());
  ...
  opState.attributes.push_back(
      {mlir::StringAttr::get(ctx_, "dims"), dimsAttr});
  opState.attributes.push_back(
      {mlir::StringAttr::get(ctx_, "range"), rangeAttr});
  opState.attributes.push_back(
      {mlir::StringAttr::get(ctx_, "args"), argsAttr});

  if (ndim > 0) {
    opState.attributes.push_back(
        {mlir::StringAttr::get(ctx_, "block_dims"),
...
mlir::Attribute OPSBuilder::buildArgAttr(const ArgDesc &arg, int ndim) {
  ...
  // Build dat shape: [size[0], size[1], ..., d_m[0], d_m[1], ..., d_p[0], d_p[1], ...]
  std::vector<int64_t> datShape;
  for (int64_t s : arg.dat.size) datShape.push_back(s);
  for (int64_t d : arg.dat.d_m) datShape.push_back(d);
...
mlir::DenseI64ArrayAttr OPSBuilder::buildDatShape(const DatDesc &dat) {
  ...
  for (int64_t s : dat.size) shape.push_back(s);
  for (int64_t d : dat.d_m) shape.push_back(d);
  for (int64_t d : dat.d_p) shape.push_back(d);
